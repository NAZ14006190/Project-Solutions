...
class RegisterController: UIViewController, UITextFieldDelegate {
    ...
    // Minimum length required for username and email
    let minLength = 6
    ...
    // Delegate function called when the user ends editing in a text field
    ...
    // Check if the text field has met the minimum length requirement for username and email
    func checkMinLengthRequirement(textField: UITextField) {
        ...
        if text.count < minLength {
            ...
            // If all fields are valid, clear the error label
            if username.layer.borderWidth == 0.0 && password.layer.borderWidth == 0.0 && email.layer.borderWidth == 0.0 {
                reglabel.text = ""
            }
        }
    }
    // Check if the text field has a strong password using regex
