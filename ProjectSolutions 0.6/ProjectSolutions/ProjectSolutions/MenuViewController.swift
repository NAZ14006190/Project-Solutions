...
class MenuViewController: UIViewController {
    ...
    // Function to be called when the edit account button is tapped
    @objc func editAccButtonTapped() {
        self.performSegue(withIdentifier: "EditAccSegue", sender: self)
       }
    // Update the status label based on the number of incomplete projects
    ...
    // Fetch the incomplete projects from the server and update the UI
    ...
    // Fetch the in progress tasks from the server and update the UI
