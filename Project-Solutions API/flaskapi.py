...
def update_user(user_id):
    try:
        ...
        conn = get_db_connection()
        conn.execute('''UPDATE users SET username=?, password=?, email=?
                        WHERE id=?''', (username, password, email, id))
        conn.commit()
        ...
        print(f"Finalising /api/users/update")
        conn.close()
...
def fetch_project_by_id(project_id):
    try:
        conn = get_db_connection()
        return SQL_to_json(conn.execute('SELECT * FROM projects WHERE id=?', (project_id,)).fetchall())
    except:
...
def fetch_tasks_by_project_id(project_id):
    try:
        ...
        return jsonify({"Error" : "No tasks to return for the specified project!"})
    finally:
        print(f"Finalising /api/tasks/get/by_project_id/{project_id}")
        conn.close()
@app.route('/api/tasks/get/by_user_id/<int:user_id>')
def fetch_tasks_by_user_id(user_id):
    try:
        conn = get_db_connection()
        tasks = conn.execute('SELECT tasks.* FROM tasks JOIN projects ON tasks.project_id = projects.id WHERE projects.user_id=?', (user_id,)).fetchall()
        return SQL_to_json(tasks)
    except:
        return jsonify({"Error" : "No tasks to return for the specified user!"})
    finally:
        print(f"Finalising /api/tasks/get/by_user_id/{user_id}")
        conn.close()
...
def update_task(task_id):
    try:
        ...
        conn = get_db_connection()
        conn.execute('''UPDATE tasks SET name=?, description=?, due_date=?, status=?, project_id=?
                        WHERE id=?''', (name, description, due_date, status, project_id, id))
        conn.commit()
