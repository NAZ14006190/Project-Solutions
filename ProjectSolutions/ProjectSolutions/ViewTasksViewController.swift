...
class ViewTasksViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    var tasks: [Task] = [] // an array to hold Task objects
    

    var projectId: Int { // a property observer to fetch task details when the project ID is set
        didSet {
            print("Project ID is set: \(projectId)")  // configure the view controller
